...
import { toggleSign, withinDigitLimit, stripGrouping, formatExpression } from './input.js';
import Icon from './Icon.jsx';
import ResultOutput from './ResultOutput.jsx';
...
export default function App() {
  ...
  const visibleResult = useMemo(
    () => completed ? result : previewExpression(expression, angle, ans),
    [completed, result, expression, angle, ans],
  );
  function input(token) {
    if (converterOpen) {
      setConverterValue(previous => {
        ...
        if (/^[0-9]$/.test(token)) { const next = previous === '0' ? token : previous + token; return withinDigitLimit(next) ? next : previous; }
        return previous;
      });
      return;
    }
    ...
    if (/^[0-9.]$/.test(token) && !withinDigitLimit(base)) { setError(korean ? '숫자는 최대 15자리까지 입력할 수 있어요.' : 'Use up to 15 digits per number.'); return; }
  ...
  async function copy() {
    ...
    try { await navigator.clipboard.writeText(formatResult(visibleResult)); setCopyStatus(labels.copied); }
  ...
  return <main className={`page ${theme}`}>
    <div className="app-wrap">
      ...
      <section className={`calculator ${mode === 'Scientific' ? 'scientific' : ''} ${converterOpen || mode === 'Scientific' ? 'expanded' : ''}`} aria-label="Calculator">
        ...
        <div className={`display-region ${drawer ? 'history-open' : ''}`}><div className="display"><input aria-label="Expression" spellCheck="false" placeholder="0" value={editingExpression ? expression : formatExpression(expression)} onFocus={() => setEditingExpression(true)} onBlur={() => setEditingExpression(false)} onChange={e => { const next = stripGrouping(e.target.value); if (!withinDigitLimit(next)) { setError(korean ? '숫자는 최대 15자리까지 입력할 수 있어요.' : 'Use up to 15 digits per number.'); return; } setExpression(next); setCompleted(false); setError(''); }} onKeyDown={e => { if (e.key === 'Enter' || e.key === '=') { e.preventDefault(); calculate(); } if (e.key === 'Escape') clear(); }}/><ResultOutput>{visibleResult === null ? '…' : formatResult(visibleResult)}</ResultOutput><div className="error" role="status">{error}</div></div>
                <button className="drawer-tab" aria-label={drawer ? 'Close history' : 'Open history'} onClick={() => setDrawer(!drawer)}>{drawer ? '‹' : '›'}</button>
        {drawer && <aside id="history" className="history" aria-label="Calculation history"><div className="history-list">{history.length ? history.map(entry => <button className="history-entry" key={entry.id} onClick={() => reuse(entry.result)}><span>{formatExpression(entry.expression)}</span><strong>{formatResult(entry.result)}</strong><small>{entry.angle}</small></button>) : <p>Your calculations<br/>will appear here.</p>}</div><button className="clear-history" disabled={!history.length} onClick={() => setHistory([])}>{labels.clearHistory}</button></aside>}</div>
        <nav className="toolbar" aria-label="Calculator utilities"><button aria-label={korean ? "단위 변환" : "Unit converter"} aria-expanded={converterOpen} aria-controls="unit-converter" onClick={() => { setConverterOpen(!converterOpen); setMode('Standard'); setDrawer(false); }}><Icon name="ruler" /></button><button className="scientific-toggle" aria-label="Scientific mode" title={mode === 'Scientific' ? 'Switch to Standard' : 'Switch to Scientific'} aria-pressed={mode === 'Scientific'} onClick={() => { setMode(mode === 'Scientific' ? 'Standard' : 'Scientific'); setConverterOpen(false); }}><Icon name="scientific" /></button><button disabled={visibleResult === null} className="copy-button" data-copied={copyStatus === labels.copied} aria-label="Copy result" onClick={copy}><Icon name="copy" /></button><span role="status">{copyStatus}</span><button aria-label="Backspace" onClick={backspace}><Icon name="backspace" /></button></nav>
        <div hidden={!converterOpen}><UnitConverter value={converterValue} onChange={value => { const next = stripGrouping(value); if (withinDigitLimit(next)) setConverterValue(next); }} korean={korean} onUse={value => { reuse(value); setConverterOpen(false); }} /></div>
