...
export default function UnitConverter({ korean, onUse, value, onChange }) {
  ...
  return <section id="unit-converter" className="converter" aria-label={korean ? '단위 변환' : 'Unit converter'}>
    ...
    <label>{korean ? '입력값' : 'Value'}<input type="text" inputMode="none" value={value} onChange={e => onChange(e.target.value)} /></label>
